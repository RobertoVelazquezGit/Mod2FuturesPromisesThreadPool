    ...
    maxThreads_(maxThreads)
{
    ...
    std::cout
        << "Dynamic thread pool initialized with "
        << minThreads
        << " threads (max: "
        << maxThreads
        << ")"
...
void DynamicThreadPool::workerThread()
{
    while (true)
    {
...

        {
            ...
            task = std::move(
                const_cast<Task&>(taskQueue_.top())
            );
...
void DynamicThreadPool::updatePerformanceMetrics(
    double taskDuration)
{
...
void DynamicThreadPool::scaleThreadPool()
{
    ...
    // and we have capacity.
    if (queueSize > current * 2
        && current < maxThreads_.load())
    {
        addWorkerThread();
    }
...
void DynamicThreadPool::addWorkerThread()
{
    workers_.emplace_back(
        &DynamicThreadPool::workerThread,
        this
    );
...
DynamicThreadPool::PoolStats
DynamicThreadPool::getStats() const
{
    return PoolStats{
        currentThreads_.load(),
        activeThreads_.load(),
