...
public:
    ...
    auto submitTask(Func&& func, Args&&... args) -> std::future<T>
    {
        ...
        std::thread([task]() {
            ...
            {
                // Exception is automatically captured by packaged_task.
            }
        }).detach();

        {
            ...
            // The future is moved into the manager. The manager becomes its owner.
    ...
    std::vector<T> waitForAll(std::chrono::milliseconds timeout = std::chrono::milliseconds(5000))
    {
        ...
        for (auto& future : activeTasks_)
        {
            try
            {
                ...
                {
                    // get() consumes the future and retrieves the result. It can only be called once.
            ...
            {
                // future.get() rethrows exceptions generated by the asynchronous task.
